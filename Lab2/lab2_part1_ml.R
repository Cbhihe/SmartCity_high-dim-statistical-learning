...
(C <- t(X) %*% X)                   # X' X ,  where X' is the transpose of X 

(X.pseudo <- solve(C) %*% t(X))       # (X' X)^{-1} X'^'
...
lines (seq(10,20,length.out=N), col="red")

#############################################
## 2. Solution via the SVD  (singular value decomposition)
#############################################
...
# Now we check that X = U D V' (it is indeed a valid decomposition)
...
D <- diag(1/s$d)  # calculate inverse of diagonal matrix
w.svd <- s$v %*% D %*% t(s$u) %*% t
...
## ... because in forming the X' X matrix some information may be lost
